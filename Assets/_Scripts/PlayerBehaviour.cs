...
public class PlayerBehaviour : MonoBehaviour {
    ...
    public float jumpStartHeight = 0f;
    public float jumpCurrentHeight = 0f;
    ...
    private void _Move() {
        if (isGrounded) {
            ...
            if (Input.GetAxisRaw("Jump") > 0.0f /*&& !isJumping*/) {
                isGrounded = false;
                isJumping = true;
                jumpStartHeight = transform.position.y;
                body.velocity = transform.up * jumpHeight * Time.deltaTime;
            }
        } else if (isJumping) {
            jumpCurrentHeight = transform.position.y;
            
            isGrounded = false;
            body.isFalling = true;

            body.velocity = playerCam.transform.up * airSpeed * Time.deltaTime;
            
            if (transform.position.y - jumpStartHeight > jumpHeight) {
                isJumping = false;
            }
